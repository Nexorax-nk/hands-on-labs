...
# Note: The original main loop was commented out to use the newer MODULAR DRIVE LOGIC below.
...
TARGET_SPEED = 150  # Target speed in km/h. Increased to 150 for Task 3/4.
...
ADAPTIVE_SPEED_CONTROL = True  # Enable advanced control logic to prevent crashes at high speeds.

# ================= HELPER FUNCTIONS =================
def calculate_adaptive_target_speed(S):
    """
    Dynamically scales the target speed based on the straightness of the road ahead
    using the center range finder (track sensor 9).
    """
    if not ADAPTIVE_SPEED_CONTROL:
        return TARGET_SPEED
        
    front_distance = S['track'][9]
    
    # If the road ahead is short (< 80 meters), we are approaching a curve
    if front_distance < 80:
        # Scale speed down proportionally, but maintain a safe minimum speed of 50 km/h
        factor = front_distance / 80.0
        dynamic_speed = max(50.0, TARGET_SPEED * factor)
    else:
        dynamic_speed = TARGET_SPEED
        
    # Further slow down if the car is currently misaligned with the track axis
    angle_factor = math.cos(S['angle'])
    dynamic_speed *= max(0.6, angle_factor)
    
    return dynamic_speed

def calculate_steering(S):
    """
    Computes steering angle based on yaw deviation (angle) and lateral position (trackPos).
    At higher speeds, steering gain is scaled down to prevent oversteering and spin-outs.
    """
    speed_factor = 1.0
    if ADAPTIVE_SPEED_CONTROL and S['speedX'] > 70:
        # Prevent high-speed spinouts by reducing steering aggressiveness
        speed_factor = 70.0 / S['speedX']
        
    steer = (S['angle'] * (STEER_GAIN * speed_factor) / math.pi) - (S['trackPos'] * CENTERING_GAIN)
    return max(-1, min(1, steer))

def calculate_throttle(S, R, dynamic_target_speed):
    """
    Controls acceleration. Tries to match the calculated dynamic target speed.
    """
    if S['speedX'] < dynamic_target_speed - (R['steer'] * 2.5):
...
def apply_brakes(S, dynamic_target_speed):
    """
    Applies braking when approaching corners or when current speed is much higher
    than the dynamic target speed.
    """
    if not ADAPTIVE_SPEED_CONTROL:
        return 0.3 if abs(S['angle']) > BRAKE_THRESHOLD else 0.0
        
    # Active braking if current speed exceeds target speed by 15 km/h
    if S['speedX'] > dynamic_target_speed + 15:
        # Proportional brake force
        brake = (S['speedX'] - dynamic_target_speed) / 40.0
        return max(0.1, min(0.7, brake))
        
    # Angle-based braking as fallback
...
def drive_modular(c):
    ...
    # 1. Compute dynamic target speed based on track curvature
    dynamic_target_speed = calculate_adaptive_target_speed(S)
    
    # 2. Compute steer, throttle, and brake using the dynamic target speed
    R['steer'] = calculate_steering(S)
    R['accel'] = calculate_throttle(S, R, dynamic_target_speed)
    R['brake'] = apply_brakes(S, dynamic_target_speed)
    
    # 3. Apply safety overrides (Traction Control & Gear Shifting)
